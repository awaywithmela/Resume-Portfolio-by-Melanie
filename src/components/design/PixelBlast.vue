...
const createLiquidEffect = (texture, opts, EffectClass) => {
  const fragment = `
...
const initThree = async (container, props) => {
  const canvas = document.createElement('canvas')
  ...
  if (props.liquid || props.noiseAmount > 0) {
    try {
      const Postprocessing = await import('postprocessing')
      const { EffectComposer, RenderPass, EffectPass, Effect } = Postprocessing
      ...
      console.warn('Failed to load postprocessing, running standard render path:', err)
    }
  }
  ...
  let raf = null
  const animate = () => {
    if (props.autoPauseOffscreen && !visibilityRef.value.visible && false) {
  ...
  return {
    ...
    updateProps: (newProps) => {
      uniforms.uShapeType.value = SHAPE_MAP[newProps.variant] ?? 0
      uniforms.uPixelSize.value = newProps.pixelSize * renderer.getPixelRatio()
      uniforms.uColor.value.set(newProps.color)
      ...
      uniforms.uEnableRipples.value = newProps.enableRipples ? 1 : 0
      uniforms.uRippleIntensity.value = newProps.rippleIntensityScale
      uniforms.uRippleThickness.value = newProps.rippleThickness
      uniforms.uRippleSpeed.value = newProps.rippleSpeed
      uniforms.uEdgeFade.value = newProps.edgeFade
      if (newProps.transparent) renderer.setClearAlpha(0)
      else renderer.setClearColor(0x000000, 1)

      if (liquidEffect) {
        const uStrength = liquidEffect.uniforms.get('uStrength')
        if (uStrength) uStrength.value = newProps.liquidStrength
        const uFreq = liquidEffect.uniforms.get('uFreq')
        if (uFreq) uFreq.value = newProps.liquidWobbleSpeed
      }
      if (touch) touch.radiusScale = newProps.liquidRadius
    }
  }
}
...
onMounted(() => {
  ...
  const ob = new IntersectionObserver(([entry]) => {
    visibilityRef.value.visible = entry.isIntersecting
  }, { threshold: 0 })
  ob.observe(container)

  let instance = null
  initThree(container, props).then(inst => {
    instance = inst
  })

  watch(() => [
    props.variant, props.pixelSize, props.color, props.patternScale,
    props.patternDensity, props.pixelSizeJitter, props.enableRipples,
    props.rippleIntensityScale, props.rippleThickness, props.rippleSpeed,
    props.edgeFade, props.transparent, props.liquidStrength,
    props.liquidRadius, props.liquidWobbleSpeed
  ], () => {
    if (instance) {
