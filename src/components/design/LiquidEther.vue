<template>
    <div ref="mountRef" class="w-full h-full relative overflow-hidden pointer-events-none touch-none" :class="className"
        :style="style"></div>
</template>
...
const Mouse = {
    ...
    init(container) {
        ...
        // Bind methods to 'this' context explicitly if needed, or use arrow functions where appropriate
...
class Output {
    ...
    init() {
        ...
        // Ensure paletteTex is initialized
        if (!paletteTex) {
            paletteTex = makePaletteTexture(props.colors)
        }
...
class WebGLManager {
    constructor(options) {
        this.props = options
        Common.init(options.$wrapper)
        Mouse.init(options.$wrapper)
...
onUnmounted(() => {
    ...
    if (intersectionObserverRef.value) intersectionObserverRef.value.disconnect()
    if (webglRef.value) webglRef.value.dispose()
    webglRef.value = null
})

watch(() => props.colors, (newColors) => {
    // Regenerate texture if needed
    // In this simple port we might just rely on re-creating text if strictly needed, 
    // but typically we'd update uniforms.
    // For now the existing structure is static relative to palette texture creation in constructor.
    // If dynamic updates are needed, we can update the uniform value.
    if (webglRef.value && webglRef.value.output) {
        paletteTex = makePaletteTexture(newColors)
        webglRef.value.output.output.material.uniforms.palette.value = paletteTex
    }
}, { deep: true })
